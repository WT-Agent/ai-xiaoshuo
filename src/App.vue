  ...
  <div class="app-container">
    ...
    <main ref="inputCardRef" class="glass-card input-group">
      ...
      <div class="options-row" style="display: grid; grid-template-columns: repeat(auto-fit, minmax(200px, 1fr)); gap: 1rem;">
        <div class="selector-group">
          <label class="selector-label">小说流派</label>
          <div class="style-selector">
            <button 
        ...
        <div class="selector-group">
          <label class="selector-label">大纲类型</label>
          <div class="style-selector">
            <button 
      ...
      <div class="selector-group">
        <div style="display: flex; justify-content: space-between; align-items: center;">
          ...
          <div style="display: flex; gap: 0.5rem;">
            ...
            <button class="text-link-btn" @click="showGuideModal = true">网文小说构思与长篇大纲通关指南</button>
          </div>
        </div>
        <textarea 
          v-model="userInput" 
...
const appTitle = ref(appConfig.title || '网腾无限AI - 网络小说大纲与剧情专家');
...
const metricsList = [
  {
    "key": "worldbuildingOriginality",
    "label": "世界观设定新颖度"
  },
  {
    "key": "characterMotivationDepth",
    "label": "人物塑造丰满深度"
  },
  {
    "key": "plotConflictEscalation",
    "label": "剧情冲突升级节奏"
  },
  {
    "key": "chapterOutlineFeasibility",
    "label": "章节细纲落地性"
  },
  {
    "key": "commercialReadershipAttraction",
    "label": "读者留存吸引分"
  }
];
const guideRules = [
  {
    "title": "主角动机必须强烈",
    "advice": "给主角设定终极目标（如复仇、救至亲、探索世界真相），让主角主动推动剧情发展",
    "avoid": "切忌主角随波逐流毫无主动性"
  },
  {
    "title": "力量体系不可崩塌",
    "advice": "升级体系要设定严格的门槛与代价，跨级打怪必须依靠战术、神器或金手指机制",
    "avoid": "避免后期战力水肿导致战力体系彻底崩坏"
  },
  {
    "title": "分卷高潮交替抑扬",
    "advice": "每一卷结尾安排一次大型高潮事件（如宗门大比/决战幕后黑手），高潮后给读者喘息复盘期",
    "avoid": "避免全程紧绷无休止打斗导致阅读疲劳"
  }
];
...
const displayResultText = computed(() => {
  ...
  return result.value.replace(/\[XIAOSHUO_SCORES\][\s\S]*?\[\/XIAOSHUO_SCORES\]/g, '').trim();
});

const parseAiScores = (rawText: string) => {
  const match = rawText.match(/\[XIAOSHUO_SCORES\](.*?)\[\/XIAOSHUO_SCORES\]/);
...
const loadHistory = () => {
  try {
    const raw = localStorage.getItem('xiaoshuo_history_records');
...
const saveHistory = () => {
  localStorage.setItem('xiaoshuo_history_records', JSON.stringify(historyList.value));
};
